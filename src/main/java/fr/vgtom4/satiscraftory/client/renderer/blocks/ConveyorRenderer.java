...
public class ConveyorRenderer implements BlockEntityRenderer<ConveyorBlockEntity> {
    ...
    @Override
    public void render(ConveyorBlockEntity conveyorBlockEntity, float partialTick, PoseStack pose, MultiBufferSource buffer, int combinedOverlay, int packedLight) {
        Item[] items = conveyorBlockEntity.getItems();
        float progress = conveyorBlockEntity.getProgress();
        Quaternion rotation = conveyorBlockEntity.getBlockState().getValue(ConveyorBlock.FACING).getRotation();
        pose.pushPose();
        ...
        for(int i = 0; i < items.length; i++) {
            ...
            if(item != null) {
                ...
                pose.translate(0, (i+progress) * (1f/ConveyorBlockEntity.itemPerConveyor) - 0.5f, -0.2f);
                itemRenderer.renderStatic(item.getDefaultInstance(), ItemTransforms.TransformType.FIXED, combinedOverlay, packedLight, pose, buffer,(int) conveyorBlockEntity.getBlockPos().asLong() + i);
                pose.popPose();
            }
        }
        pose.popPose();
