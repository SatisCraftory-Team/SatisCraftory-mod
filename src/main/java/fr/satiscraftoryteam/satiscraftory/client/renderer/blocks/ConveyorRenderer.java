...
public class ConveyorRenderer implements BlockEntityRenderer<ConveyorTileEntity> {
    ...
    public void render(ConveyorTileEntity conveyorTileEntity, float partialTick, PoseStack pose, MultiBufferSource buffer, int combinedOverlay, int packedLight) {
        ItemStack[] items = conveyorTileEntity.getItems();
        ...
        for(int i = 0; i < items.length; i++) {
            ...
            if(item != null) {
                ...
                itemRenderer.renderStatic(item, ItemDisplayContext.FIXED, combinedOverlay, packedLight, pose, buffer, conveyorTileEntity.getLevel(), (int) conveyorTileEntity.getBlockPos().asLong() + i);
                pose.popPose();
            }
        }
        pose.popPose();
